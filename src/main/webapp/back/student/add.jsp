...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-sm-6 col-sm-offset-3">
            <form class="form-horizontal" action="${pageContext.request.contextPath}/student/insertStudent" method="post">
...
<script>
    $(function () {
        //发送ajax请求查询所有城市信息
        $.get("${pageContext.request.contextPath}/city/findAllCity",function(res){
            console.log(res);
            //遍历城市信息
        ...
        //发送ajax请求查询标签信息(学生类型的标签)
        $.post("${pageContext.request.contextPath}/tag/selectByType",{type:'学生'},function(tags){
            console.log(tags);
            $.each(tags,function(index,tag){
                let label = $("<label/>");
                let cks = $("<input/>").attr({type:"checkbox",name:"tagIds"}).val(tag.tid);
                label.append(cks).append(tag.tname +"&nbsp;&nbsp;");
                $("#cks").append(label);
            });
        });

        //发送ajax请求查询所有班级信息
        $.get("${pageContext.request.contextPath}/class/findAllClass",function(clazzs){
        ...
        //监听班级的下拉列表中onchange事件根据选择班级获取对应的小组信息
        $("#clazz").change(function(){
            //清空上一次小组信息
            $("#group").empty();
            $.post("${pageContext.request.contextPath}/group/findByClass",{clazzid:$("#clazz").val()},function(res){
